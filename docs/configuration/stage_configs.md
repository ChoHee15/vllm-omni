...
| `devices` | str \| null | optional | `null` | Device list assigned to a single stage process. The number of device ids must equal this stage's per-process width (`tensor_parallel_size` × `pipeline_parallel_size`, or `num_replicas` × that product for a replica pool); a mismatch fails early — see the note below. |
...
**Note:** a stage's `devices` count must equal its per-process device width — `tensor_parallel_size` × `pipeline_parallel_size` (both are intra-engine dimensions), or `num_replicas` × `tensor_parallel_size` × `pipeline_parallel_size` for a replica pool. `data_parallel_size` is *not* included: data parallelism spawns additional engine processes rather than widening one process's device list. A top-level `--tensor-parallel-size` is broadcast to every stage, so a single-GPU stage can violate this; it now fails early naming the offending stage ([issue #5003](gh-issue:5003)) — fix with `--stage-overrides` (set `tensor_parallel_size` and `devices` together per stage) or set tp only on the multi-GPU stage.
